...
<#
.Synopsis
    This PowerShell is setting registry values for additonal script to define which update policy the client system will get assigned by AAD Group
    IMPORTANT: This script does not reboot the system to apply or query system.
    IMPORTANT: Dell Command | Update need to install first on the devices.

.DESCRIPTION
   PowerShell setting/maintaining UpdateRings for Dell Command | Update based on AAD Groups
#>
...
If ($CheckRegMainPath -eq $true)
        ...
        if ($CheckRegAppPath -eq $true)
            {
                ...
                Write-Host "Path $RegMainPath is available"
                New-ItemProperty -Path $RegAppPath -Name UpdatePolicy -Value $DCURing -Force
                New-ItemProperty -Path $RegAppPath -Name UpdateSetTime -Value $TimeStamp -Force  
        ...
        else 
            {
                ...
                New-Item -Path $RegAppPath -Force
                New-ItemProperty -Path $RegAppPath -Name UpdatePolicy -Value $DCURing -Force
                New-ItemProperty -Path $RegAppPath -Name UpdateSetTime -Value $TimeStamp -Force 
...
else 
    {
        ...
        New-Item -Path $RegMainPath -Force
        New-Item -Path $RegAppPath -Force
        New-ItemProperty -Path $RegAppPath -Name UpdatePolicy -Value $DCURing -Force
        New-ItemProperty -Path $RegAppPath -Name UpdateSetTime -Value $TimeStamp -Force 
